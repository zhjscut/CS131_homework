...
class PCA(object):
    ...
    def fit(self, X, method='svd'):
        ...
        # X_centered=X_centered/np.linalg.norm(X_centered,axis=0)
        if method=='svd':
    ...
    def _eigen_decomp(self, X):
        ...
        indics=np.argsort(e_vals)
        indics=np.flip(indics,axis=0)
        e_vals=e_vals[indics]
        e_vecs=e_vecs[:,indics]
        # END YOUR CODE
    ...
    def _svd(self, X):
        ...
        N, D = X.shape
        vecs = np.empty((D,D))  # shape (D, D)
        vals = None  # shape (K,)
        # YOUR CODE HERE
        # Here, compute the SVD of X
        # Make sure to return vecs as the matrix of vectors where each column is a singular vector
        [u,s,v]=np.linalg.svd(X)##这里得出来的v就已经是装置了
        indics=np.argsort(s)
        indics=np.flip(indics,axis=0)
        vals=s[indics]
        vecs[:,:indics.shape[0]]=v[:,indics]
        vecs[:,indics.shape[0]:]=v[:,indics.shape[0]:]
        vecs=vecs.T
        # END YOUR CODE
...
class LDA(object):
    ...
    def fit(self, X, y):
        ...
        e_vals,e_vecs=scipy.linalg.eig(inv.dot(scatter_between))
        sort_val_indics=np.argsort(e_vals)
        sort_val_indics=np.flip(sort_val_indics,axis=0)
        e_vecs=e_vecs[:,sort_val_indics]
        # END YOUR CODE
    ...
    def _within_class_scatter(self, X, y):
        ...
        for i in np.unique(y):
            ...
            X_i=X[y==i]
            X_i_center=X_i-np.mean(X_i,axis=0)
            S_i=(X_i_center.T).dot(X_i_center)
    ...
    def _between_class_scatter(self, X, y):
        ...
        for i in np.unique(y):
            ...
            var_i=N_i*((mu_i-mu).T).dot(mu_i-mu)
            scatter_between = scatter_between+var_i
    ...
    def transform(self, X, n_components):
        ...
        prin_lda=self.W_lda[:,:n_components]
        X_proj=X.dot(prin_lda)
        # END YOUR CODE
