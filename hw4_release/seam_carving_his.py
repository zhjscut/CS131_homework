...
def compute_cost(image, energy, axis=1):
    ...
    ### YOUR CODE HERE
    for i in range(1,H):
        center = cost[i-1, :].astype(float)
        left = np.hstack([[100.], cost[i-1, 0:-1]]).astype(float)
        right = np.hstack([cost[i-1, 1:], [100.]]).astype(float)
        temp=np.vstack([left,center,right])
        cost[i, :] = energy[i, :] + temp.min(0)
...
def remove_seam(image, seam):
    ...
    for i in range(H):
        for j in range(C):
            out[i, :, j] = np.delete(image[i, :, j], seam[i])
    pass
    ### END YOUR CODE
...
def reduce(image, size, axis=1, efunc=energy_function, cfunc=compute_cost):
    ...
    assert out.shape[1] == size, print("Output doesn't have the right shape",out.shape[1] ,size)
    if axis == 0:
        out = np.transpose(out, (1, 0, 2))
...
def enlarge_naive(image, size, axis=1, efunc=energy_function, cfunc=compute_cost):
    ...
    for _ in range(size-W):
        image = duplicate_seam(image, seam)
    out = image
    pass
    ### END YOUR CODE
...
def find_seams(image, k, axis=1, efunc=energy_function, cfunc=compute_cost):
    ...
    for i in range(k):
        ...
        temp = indices[np.arange(H), seam].astype(int)
        assert np.all(seams[np.arange(H), temp] == 0), \
            "we are overwriting seams"
        seams[np.arange(H), temp] = i + 1
...
def enlarge(image, size, axis=1, efunc=energy_function, cfunc=compute_cost):
    ...
    ### YOUR CODE HERE
    seams = find_seams(image, size - W)
    for i in range(size - W):
        index = np.where(seams == i+1)[1]
        image = duplicate_seam(image, index)
        seams = duplicate_seam(seams, index)
    out = image
    pass
    ### END YOUR CODE
...
def compute_forward_cost(image, energy):
    ...
    for i in range(1,H):
        '''
        center = cost[i-1,:]+np.abs(np.hstack([image[i,1:],[image[i,H-2]]])-np.hstack([[image[i,1]],image[i,0:-1]]))
        #left = np.hstack([[100.],cost[i-1,0:-1]])
        left = np.hstack([[100.],cost[i-1,0:-1]])+np.abs(np.hstack([image[i,1:],[image[i,H-2]]])-np.hstack([[image[i,1]],image[i,0:-1]]))+np.abs(np.hstack([image[i-1,:]])-np.hstack([[image[i,1]],image[i,0:-1]]))
        right = np.hstack([cost[i-1, 1:], [100.]])+np.abs(np.hstack([image[i,1:],[image[i,H-2]]])-np.hstack([[image[i,1]],image[i,0:-1]]))+np.abs(np.hstack([image[i-1,:]])-np.hstack([image[i,1:],[image[i,H-2]]]))
        temp = np.vstack([left,center,right])
        
        cost[i,:] = energy[i,:]+temp.min(axis=0)
        paths[i,:] = np.argmin(temp,axis=0)-1
        '''
        image_temp = np.hstack([image[i-1:i+1, 1:2], image[i-1:i+1, :], image[i-1:i+1, -2:-1]])
        
        center = cost[i-1, :].astype(float)
        Cv = np.abs(image_temp[1, :W] - image_temp[1, 2:])
        center += Cv
        
        left = np.hstack([[100.], cost[i-1, 0:-1]]).astype(float)
        Cl = np.abs(image_temp[1, :W] - image_temp[1, 2:]) + np.abs(image_temp[0, 1:-1] - image_temp[1, :W])
        # print(np.abs(image_temp[1, :W] - image_temp[1, 2:]))
        # print(Cl.shape)
        left += Cl
        
        right = np.hstack([cost[i-1, 1:], [100.]]).astype(float)
        Cr = np.abs(image_temp[1, :W] - image_temp[1, 2:]) + np.abs(image_temp[0, 1:-1] - image_temp[1, 2:])
        right += Cr
        
        temp=np.vstack([left,center,right])
        cost[i, :] = temp.min(0) + energy[i, :]
        paths[i, :] = np.argmin(temp, axis=0) - 1
