...
def compute_cost(image, energy, axis=1):
    ...
    if axis == 0:##这里有点奇怪，axis==0是找水平线。1才是找垂直线
    ...
    ### YOUR CODE HERE
    last_cost=np.zeros((3,W))
    for i in range(1,H):
        
        last_cost[0,1:]=cost[i-1,0:-1]
        last_cost[1]=cost[i-1,:]
        last_cost[2,:-1]=cost[i-1,1:]
        last_cost[0,0]=cost[i-1,0]+cost[i-1,1]+1
        last_cost[-1,-1]=cost[i-1,-1]+cost[i-1,-2]+1
        min_index=np.argmin(last_cost,axis=0)
        cost[i]=energy[i]+np.amin(last_cost,axis=0)
        paths[i]=min_index-1
...
def backtrack_seam(paths, end):
    ...
    ### YOUR CODE HERE
    for i in range(H-1):
        seam[H-2-i]=seam[H-1-i]+paths[H-1-i,seam[H-1-i]]
    ### END YOUR CODE

    # Check that seam only contains values in [0, W-1]
    assert np.all(np.all([seam >= 0, seam < W], axis=0)), "seam contains values out of bounds,maxseam=%d" %np.max(seam)
...
def reduce(image, size, axis=1, efunc=energy_function, cfunc=compute_cost):
    ...
    print('total remove time is ',remove_time)
...
def enlarge_naive(image, size, axis=1, efunc=energy_function, cfunc=compute_cost):
    ...
    for i in range(size-W):
        energy = efunc(image)
        vcost,vpath=cfunc(image,energy,axis=axis)
        end = np.argmin(vcost[-1])
...
def find_seams(image, k, axis=1, efunc=energy_function, cfunc=compute_cost):
    ...
    '''大体思路：关键点是用一个indices保存原图像坐标的索引，当从原图像remove之后，indices也remove，即被移走的坐标
    不会再次被移走。
    '''
    ...
    for i in range(k):
        ...
        assert np.all(seams[np.arange(H,dtype=int), indices[np.arange(H,dtype=int), seam]] == 0), \
            "we are overwriting seams"
        seams[np.arange(H,dtype=int), indices[np.arange(H,dtype=int), seam]] = i + 1
...
def enlarge(image, size, axis=1, efunc=energy_function, cfunc=compute_cost):
    ...
    ### YOUR CODE HERE
    seams = find_seams(out,size-W)#实际上已经事先对image翻转成out 了，所以这里的axis不能使用成0，或者变量要变成image
    for i in range(size-W):
        seam = np.argwhere(seams==i+1)[:,1]
        out = duplicate_seam(out,seam)
...
def compute_forward_cost(image, energy):
    ...
    ### YOUR CODE HERE
    forward_cost=np.empty((3,W))
    g2=np.empty((W,))
    g3=np.empty((W,))
    for i in range(1,H):        
        g1=np.abs(np.gradient(image[i,:]))*2
        g1[0]=image[i,1]
        g1[-1]=image[i,-2]

        g2[1:]=np.abs(image[i-1,1:]-image[i,:-1])
        g3[:-1]=np.abs(image[i-1,:-1]-image[i,1:])
        forward_cost[0]=g1+g2+np.hstack(([0],cost[i-1,0:-1]))
        forward_cost[1]=g1+cost[i-1]
        forward_cost[2]=g3+g1+np.hstack((cost[i-1,1:],[0]))
        forward_cost[0,0]=forward_cost[1,0]+forward_cost[2,0]+1
        forward_cost[-1,-1]=forward_cost[0,-1]+forward_cost[1,-1]+1

        cost[i]=energy[i]+np.amin(forward_cost,axis=0)
        paths[i]=np.argmin(forward_cost,axis=0)-1
...
def remove_object(image, mask):
    ...
    out= reduce(out,W-int(len*1.5),efunc=lambda x:(energy),cfunc=lambda x,y:(cost,paths))
    out= enlarge(out,W)
    ### END YOUR CODE
